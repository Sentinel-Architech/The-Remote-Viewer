# Scaffold → LIVE / PROVEN

Authority: [REALITY.md](REALITY.md).  
**Updated:** 2026-08-20

## LIVE (hosted)

Viewer Hub DApp — [`apps/hub`](../apps/hub) · briefing · daily watch · profile vault · public card

This is **not** the old `apps/web` scaffold.

## PROVEN

Optical TRVL · Path B · verifier · recognition · local models/UI/defense

## DEMONSTRATED (2026-08-14)

| ID | Note |
|----|------|
| **B2** | PWA on phone |
| **H1–H3** | Learning persist / Reset wipe / no upload |
| **C partial** | Termux probe Pixel 7 API37 localRuntime cam/mic; Keystore **open** |

## Open

| ID | Needs |
|----|--------|
| **A** | Build host `anchor build` |
| **C full** | Native Android Keystore + mapTier wiring |
| **D** | Wear OS device |
| **E** | After A |
| **F / G** | Rails / real handoff |
| **Mobile Expo** | Still PARKED — not the hub |

**Next hard gate (chain): A.**  
Hub is already live; do not list it as open.
