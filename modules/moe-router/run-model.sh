...
# Stage B: select by tag and invoke local backend (or stub)
set -euo pipefail
...
if [[ "$BACKEND" == "llama.cpp" ]]; then
  if [[ -z "$MPATH" || ! -f "$MPATH" ]]; then
    ...
    echo "Place model file or edit modules/moe-router/models.json" >&2
    exit 3
  fi
  CLI="${LLAMA_CLI:-}"
  if [[ -z "$CLI" ]]; then
  ...
  if [[ -z "$PROMPT" ]]; then
    echo "Selected only (no prompt). Example:"
    echo "  $0 $TAG \"hello\""
    exit 0
  fi
  # Minimal local invoke — flags vary by llama.cpp build; override via LLAMA_ARGS
  # shellcheck disable=SC2086
  exec "$CLI" -m "$MPATH" -p "$PROMPT" ${LLAMA_ARGS:- -n 128 --no-display-prompt}
fi
