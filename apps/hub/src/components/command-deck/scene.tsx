...
import { healTier, isLearned, learnedCount, sightTier, rankFor, sigKey, useProgress } from "@/lib/command-deck/progress";
...
type Grab = {
  ...
  last: THREE.Vector3;
  throwVel: THREE.Vector3;
};

const grab: { current: Grab | null } = { current: null };
const orbit: { current: { enabled: boolean } | null } = { current: null };
...
function setOrbitEnabled(enabled: boolean) {
  if (orbit.current) orbit.current.enabled = enabled;
}

function GrabController() {
  const setGrabbing = usePlayground((s) => s.setGrabbing);
  const { camera, pointer, gl, controls } = useThree();
  const raycaster = useThree((s) => s.raycaster);
  orbit.current = (controls as unknown as { enabled: boolean } | null) ?? null;
  ...
  useBeforePhysicsStep(() => {
    ...
    _target.copy(raycaster.ray.direction).multiplyScalar(g.depth).add(camera.position);
    _target.add(g.offset);
    g.throwVel.copy(_target).sub(g.last).multiplyScalar(60);
    g.last.copy(_target);
    g.body.setNextKinematicTranslation(_target);
  });
  ...
  useEffect(() => {
    const end = () => {
      ...
      if (g.body.isValid()) {
        ...
        _target.copy(raycaster.ray.direction).multiplyScalar(g.depth).add(camera.position);
        _target.add(g.offset);
        _vel.copy(_target).sub(g.last).multiplyScalar(60);
        if (_vel.lengthSq() < g.throwVel.lengthSq()) _vel.copy(g.throwVel);
        const speed = _vel.length();
        const cap = usePlayground.getState().theater === "neural" ? 9.5 : 22;
        if (speed > cap) _vel.multiplyScalar(cap / speed);
        g.body.setBodyType(RigidBodyType.Dynamic, true);
        g.body.setLinvel({ x: _vel.x, y: _vel.y, z: _vel.z }, true);
      }
      grab.current = null;
      setGrabbing(false);
      setOrbitEnabled(true);
  ...
  }, [camera, gl, pointer, raycaster, setGrabbing]);
...
function beginGrab(
  ...
  grab.current = {
    body,
    depth,
    offset: new THREE.Vector3(pos.x - hit.x, pos.y - hit.y, pos.z - hit.z),
    last: new THREE.Vector3(pos.x, pos.y, pos.z),
    throwVel: new THREE.Vector3(),
  };
  setOrbitEnabled(false);
  usePlayground.getState().setGrabbing(true);
  usePlayground.getState().discover();
  usePlayground.getState().markSeize(id);
      ...
      <group>
        <mesh castShadow receiveShadow>
          ...
          <meshPhysicalMaterial
            ...
            emissive={color}
            emissiveIntensity={sentinel ? 0.35 : orbit ? 0.22 : 0.08}
            sheen={orbit ? 0 : 0.4}
            sheenColor={color}
          />
        </mesh>
...
function Body({ body, restitution }: { body: SpawnedBody; restitution: number }) {
  ...
  return (
    ...
    >
      ...
      <group
        onPointerOver={() => {
          if (!usePlayground.getState().grabbing) gl.domElement.style.cursor = "grab";
        }}
        onPointerOut={() => {
          if (!usePlayground.getState().grabbing) gl.domElement.style.cursor = "auto";
        }}
        onPointerDown={(e) => {
          const rb = ref.current;
          if (!rb) return;
          beginGrab(rb, e, gl.domElement, body.id);
        }}
      >
...
function SceneTint() {
  ...
  useEffect(() => {
    ...
    if (theater === "neural") {
      ...
      scene.fog = new THREE.FogExp2(NEURAL_FOG, 0.052 - heal * 0.01);
    } else {
      gl.setClearColor(ORBIT_FOG, 1);
      scene.fog = new THREE.Fog(ORBIT_FOG, 14 + sight * 4, 48 + sight * 10);
    }
  }, [theater, healed, cleared, xp, gl, scene]);
  return null;
}
...
function World() {
  ...
  return (
    ...
    >
      ...
      {neural ? null : <Stars radius={48} depth={24} count={2200} factor={3.2} saturation={0} fade speed={0.35} />}
