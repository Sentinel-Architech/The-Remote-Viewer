...
export function UserButton() {
  ...
  return (
    <div className="flex items-center gap-1 sm:gap-2">
      ...
      <span className="hidden max-w-[9rem] truncate text-sm font-medium md:inline">{label}</span>
      {authEnabled && (
        <button
          type="button"
          onClick={() => void signOut()}
          className="cursor-pointer text-xs underline-offset-4 opacity-70 hover:underline sm:text-sm"
        >
          Out
        </button>
      )}
