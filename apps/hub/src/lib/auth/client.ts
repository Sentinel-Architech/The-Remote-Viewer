...
export const authClient = createAuthClient({
  ...
  fetchOptions: {
    ...
    onSuccess(ctx) {
      // Email/password (and any other cookie session) also expose the signed
      // token via the bearer plugin. Capture it so live-preview iframes, which
      // cannot keep `__Host-` cookies, still have a session after register.
 ...
 *
 * Also treat ANY iframe as preview: Google/X refuse to render inside frames, and
 * some preview hosts are not `*.grok-sandbox.com`. Redirecting the iframe itself
 * is what made "Continue with Google / X" look like a dead connection.
 */
...
function inLivePreview(): boolean {
  return (
    ...
    (window.location.hostname.endsWith(".grok-sandbox.com") || inIframe())
  );
}
...
export async function signIn(
  ...
  if (inLivePreview()) {
    if (!popup) {
      throw new Error(
        "Pop-up blocked — allow pop-ups, then try Google/X again, or register with a native email lock.",
      );
    }
