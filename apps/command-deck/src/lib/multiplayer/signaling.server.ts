...
function ensureSchema(client: Sql): Promise<void> {
...
async function roster(client: Sql, room: string): Promise<PeerRow[]> {
  const rows = await client.query<{ peer_id: string; name: string }>(
...
async function touchPeer(client: Sql, room: string, peer: string, name: string) {
  await client.query(
...
async function prune(client: Sql) {
  await Promise.all([
    client.query(`DELETE FROM webrtc_signals WHERE created_at < now() - make_interval(secs => $1)`, [
      SIGNAL_TTL_SECONDS,
    ]),
    client.query(`DELETE FROM webrtc_peers WHERE last_seen < now() - make_interval(secs => $1)`, [
      PEER_TTL_SECONDS,
    ]),
  ]);
}
...
async function handleGet(url: URL): Promise<Response> {
  ...
  if (since === 0 || Math.random() < 0.02) await prune(client);
  await touchPeer(client, room, peer, name);
  ...
  const body: RtcPollResponse = {
    peers: await roster(client, room),
    signals: rows.map((r) => ({
      id: r.id,
      from: r.from_peer,
      kind: r.kind,
...
async function handlePost(request: Request): Promise<Response> {
  ...
  if (msg.op === "signal") {
    ...
    await client.query(`DELETE FROM webrtc_peers WHERE room = $1 AND peer_id = $2`, [
      msg.room,
      msg.peer,
    ]);
  }
