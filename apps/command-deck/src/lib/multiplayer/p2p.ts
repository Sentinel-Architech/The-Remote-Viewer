import { isNativeIceCandidate, sanitizeIceServers } from "@/lib/native";
...
export interface P2PRoomOptions {
  ...
  selfId: string;
  name?: string;
  /** Defaults to VITE_STUN_URLS after native sanitizer, else host ICE only. */
...
function nativeCandidateInit(candidate: RTCIceCandidateInit) {
  const line = candidate.candidate || "";
  if (!line) return true;
  return isNativeIceCandidate(line);
}
...
export function defaultIceServers(): RTCIceServer[] {
  const urls = (import.meta.env.VITE_STUN_URLS as string | undefined)
    ...
    .map((u) => u.trim())
    .filter(Boolean);
  // Host ICE by default (same Wi-Fi). Env STUN is allowed only after the native
  // sanitizer strips Google / Microsoft / vendor identity servers.
  return sanitizeIceServers(urls?.length ? [{ urls }] : []);
}
...
export class P2PRoom {
  ...
  private connectTo(peerId: string, name: string, initiator: boolean): PeerSlot | null {
    ...
    const pc = new RTCPeerConnection({
      iceServers: sanitizeIceServers(this.opts.iceServers ?? defaultIceServers()),
    });
    const slot: PeerSlot = {
      pc,
      makingOffer: false,
    ...
    pc.onicecandidate = (e) => {
      ...
      if (line && !isNativeIceCandidate(line)) return;
  ...
  private async flushPendingCandidates(slot: PeerSlot): Promise<void> {
    while (slot.pendingCandidates.length > 0) {
      ...
      if (!nativeCandidateInit(candidate)) continue;
      try {
  ...
  private async onSignal(
    ...
    try {
      if (kind === "offer" || kind === "answer") {
        ...
        if (!nativeCandidateInit(candidate)) return;
