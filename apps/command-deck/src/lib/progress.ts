...
export function emptySnapshot(): Snapshot {
  return { xp: 0, healed: 0, cleared: 0, seizes: 0, watches: 0, learned: {}, seq: 0 };
}

export function parseSnapshot(raw: unknown): Snapshot | null {
  if (!raw || typeof raw !== "object") return null;
  const p = raw as Partial<Snapshot>;
  return {
    xp: Number(p.xp) || 0,
...
export function mergeSnapshots(a: Snapshot, b: Snapshot): Snapshot {
  const learned: Record<string, number> = { ...a.learned };
  for (const [k, v] of Object.entries(b.learned)) {
    learned[k] = Math.max(learned[k] ?? 0, v);
  }
  return {
    xp: Math.max(a.xp, b.xp),
    healed: Math.max(a.healed, b.healed),
    cleared: Math.max(a.cleared, b.cleared),
    seizes: Math.max(a.seizes, b.seizes),
    watches: Math.max(a.watches, b.watches),
    learned,
    seq: Math.max(a.seq, b.seq),
  };
}

export function snapshotEqual(a: Snapshot, b: Snapshot) {
  if (a.xp !== b.xp || a.healed !== b.healed || a.cleared !== b.cleared) return false;
  if (a.seizes !== b.seizes || a.watches !== b.watches || a.seq !== b.seq) return false;
  const keys = new Set([...Object.keys(a.learned), ...Object.keys(b.learned)]);
  for (const k of keys) {
    if ((a.learned[k] ?? 0) !== (b.learned[k] ?? 0)) return false;
  }
  return true;
}

export function canonDossier(pubkey: string, snap: Snapshot) {
  const learned: Record<string, number> = {};
  for (const row of SIGNATURES) {
    const n = snap.learned[row.key];
    if (n && n > 0) learned[row.key] = n;
  }
...
function bump(prev: Snapshot, patch: Partial<Snapshot>): { next: Snapshot; ranked: Rank | null } {
  const next: Snapshot = {
    ...
    seq: Math.max(Date.now(), prev.seq + 1),
  };
...
export const useProgress = create<ProgressState>((set, get) => ({
  ...
  onSnapBonus: () => {
    const { next, ranked } = bump(get(), { xp: 3 });
    set(next);
    return ranked;
  },
  onPulseUpgrade: (score) => {
    const xp = Math.min(24, 8 + Math.max(0, score));
    const { next, ranked } = bump(get(), { xp });
    set(next);
    return ranked;
  },
  applyRemote: (remote) => {
    const prev = get();
    const next = mergeSnapshots(prev, remote);
    if (snapshotEqual(prev, next)) return false;
    write(next);
    set(next);
    return true;
  },
